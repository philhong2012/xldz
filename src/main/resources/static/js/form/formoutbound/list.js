...
$(function() {
    layui.use('table', function(){
        ...
        tableIns=table.render({
            elem: '#formOutboundList'
            ,url:'/formoutbound/list'
        ...
        //监听工具条
        table.on('tool(formOutboundList)', function(obj){
            var data = obj.data;
            if(obj.event === 'del'){
    ...
    layui.use(['form','laydate'], function(){
        ...
        //日期
        laydate.render({
            elem: '#startOutboundDate'
        });
        laydate.render({
            elem: '#endOutboundDate'
        });
        //TODO 数据校验
        //监听搜索框
...
function edit(obj,id) {
    window.location.href = '/formoutbound/edit?id='+id;
}

function deleteData(obj,id) {
    $.ajax({
        "url": "/formoutbound/delete?id="+id,
        "type": "post",
        "data": null,
        "contentType": "application/json",
        "success": function (data) {
            if(data == "ok") {
                layer.alert("操作成功",{time: 500 }, function () {
                    //layer.closeAll();
