...
$(function() {
    layui.use('table', function(){
        ...
        tableIns=table.render({
            elem: '#customsClearanceList'
            ,url:'/customsclearance/list'
        ...
        //监听工具条
        table.on('tool(customsClearanceList)', function(obj){
            var data = obj.data;
            if(obj.event === 'del'){
        ...
        //监听提交
        form.on('submit(customsClearanceSubmit)', function(data){
            // TODO 校验
            formSubmit(data);
            return false;
        });
...
function exportFile() {
    ...
    if(checkStatus.data.length > 0) {
        window.location.href = '/customsclearance/download?id='+checkStatus.data[0].id;
...
function deleteData(obj,id) {
    $.ajax({
        "url": "/customsclearance/delete?id="+id,
        "type": "post",
        "data": null,
